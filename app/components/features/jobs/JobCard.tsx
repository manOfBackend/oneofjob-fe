...
const CompanyLogo = memo<{ company: string; size?: 'sm' | 'md' | 'lg' }>(
  ({ company, size = 'md' }) => {
    ...
    return (
      <div
        className={`${sizeClasses[size]} rounded-lg overflow-hidden bg-gray-50 p-2 flex-shrink-0`}
      >
        <img
          src={
            logoError
              ? '/images/companies/default.svg'
              : `/images/companies/${company.toLowerCase()}.svg`
          }
          alt={`${company} 로고`}
...
export const JobCard = memo<JobCardProps>(({ job, className = '' }) => {
  ...
  const isUrgent =
    deadlineInfo.includes('일 남음') && parseInt(deadlineInfo.match(/\d+/)?.[0] || '999') <= 7;
    ...
    >
            ...
            <div className='flex items-start justify-between gap-4'>
              <div className='flex-1 min-w-0'>
                ...
                <div className='flex flex-wrap gap-2 mt-3'>
                  {job.careers.map(career => (
                    <Badge key={career} variant={CAREER_BADGE_VARIANTS[career]} size='sm'>
                      {career}
                    </Badge>
                  ))}
              ...
              <div className='text-right flex-shrink-0'>
                ...
                <div className='text-sm text-gray-500 mt-2 space-y-1'>
                  ...
                  {job.startDate && <div>등록: {formatDate(job.startDate)}</div>}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className='block sm:hidden p-4'>
        <div className='space-y-4'>
          ...
          {/* 기간 정보 */}
          <div className='text-xs text-gray-500'>기간: {dateRangeInfoShort}</div>
